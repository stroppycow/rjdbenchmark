...
function format_span(info){
  ...
  if(info['period'] == 12){
    ...
    var serie_span_start = new Date(
      parseInt(info['serie_span_start'].substring(0, 4)),
      parseInt(info['serie_span_start'].substring(5, 7))-1,
      1
    );
    var serie_span_end = new Date(
      parseInt(info['serie_span_end'].substring(0, 4)),
      parseInt(info['serie_span_end'].substring(5, 7))-1,
      1
    );
    var estimation_span_start = new Date(
      parseInt(info['estimation_span_start'].substring(0, 4)),
      parseInt(info['estimation_span_start'].substring(5, 7))-1,
      1
    );
    var  estimation_span_end = new Date(
      parseInt(info['estimation_span_end'].substring(0, 4)),
      parseInt(info['estimation_span_end'].substring(5, 7))-1,
      1
    );
    d = {
    ...
    var  serie_span_start = new Date(
      parseInt(info['serie_span_start'].substring(0, 4)),
      parseInt(info['serie_span_start'].substring(5, 7))-3,
      1
    );
    var serie_span_end = new Date(
      parseInt(info['serie_span_end'].substring(0, 4)),
      parseInt(info['serie_span_end'].substring(5, 7))-3,
      1
    );
    var estimation_span_start = new Date(
      parseInt(info['estimation_span_start'].substring(0, 4)),
      parseInt(info['estimation_span_start'].substring(5, 7))-3,
      1
    );
    var estimation_span_end = new Date(
      parseInt(info['estimation_span_end'].substring(0, 4)),
      parseInt(info['estimation_span_end'].substring(5, 7))-3,
      1
    );
    d = {
      'serie_span': ('[' + serie_span_start.toLocaleString("fr-FR", precision) + ' , '+ serie_span_end.toLocaleString("fr-FR", precision) + ']<br>(n = '+info['serie_span_n'].toString()+')'),
...
function format_stat_jd(pvalue, qualite){
  ...
  if(pvalue === null){
    return '<p style="color:'+color+'";>'+ qualite + '</p>' ;
  } else {
...
async function extract_info_ws(ws, old_new, sa_item){
    var res = await db.query(`
        SELECT *
        FROM jd_infos
        WHERE ws_name = ? and type_ws = ? and series = ?
        `,
          [
          ws,
          old_new,
...
infos_old_sel = extract_info_ws(ws_old_sel, 'old', sa_item_old_sel) ;
infos_new_sel = extract_info_ws(ws_new_sel, 'new', sa_item_new_sel) ;
```
  ...
  var data_generalite = [
    {
      ...
      'old': ((infos_old_sel === null) ? ' - ' :  ws_old_sel),
      'new': ((infos_new_sel === null) ? ' - ' : ws_new_sel)
    },
    {
      'label':'Nom du SaItem',
      'old': ((infos_old_sel === null) ? ' - ' : sa_item_old_sel),
      'new': ((infos_new_sel === null) ? ' - ' : sa_item_new_sel)
    },
    {
      'label':'Score de qualité globale',
      'old': ((infos_old_sel === null) ? ' - ' : format_stat_jd(null, infos_old_sel['summary'])),
      'new': ((infos_new_sel === null) ? ' - ' : format_stat_jd(null, infos_new_sel['summary']))
    },
    {
      'label':'Version du cruncher utilisée',
      'old': ((infos_old_sel === null) ? ' - ' : infos_old_sel['version']),
      'new': ((infos_new_sel === null) ? ' - ' : infos_new_sel['version'])
    },
        {
      'label':'Mode de rafraichissement',
      'old': ((infos_old_sel === null) ? ' - ' : infos_old_sel['refresh_policy']),
      'new': ((infos_new_sel === null) ? ' - ' : infos_new_sel['refresh_policy'])
    },
    {
      'label':'Estimation span',
      'old': ((infos_old_sel === null) ? ' - ' : format_span(infos_old_sel)['estimation_span']),
      'new': ((infos_new_sel === null) ? ' -'  : format_span(infos_new_sel)['estimation_span'])
    },
    {
      'label':'Serie span',
      'old': ((infos_old_sel === null) ? ' - ' : format_span(infos_old_sel)['serie_span']),
      'new': ((infos_new_sel === null) ? ' - ' : format_span(infos_new_sel)['serie_span'])
    }
  ] ; 
...
function get_yaxis_range(traces, range){
    var min_date =  new Date(
      parseInt(range[0].substring(0, 4)),
      parseInt(range[0].substring(5, 7))-1,
      1
    ) ;
    var max_date =  new Date(
      parseInt(range[1].substring(0, 4)),
      parseInt(range[1].substring(5, 7))-1,
      1
    ) ;

    
    var min_max_all_visibles = traces.reduce(
        (accumulator, trace) => {
            if(trace['visible'] === true){
                var  couples = trace['x'].map((val, index) => [val, trace['y'][index]]) ;
                return couples.reduce(
                    (acc, el) => {
                        if(el[0] >= acc['min_date'] && el[0] <= acc['max_date']){
                            if(el[1] > acc['max_val']){
                               acc['max_val'] = el[1] ;
                            }
                            if(el[1] < acc['min_val']){
    ...
    if(min_max_all_visibles['min_val'] === Number.POSITIVE_INFINITY){
        return([0 , 1]);
    } else {
        return([
            min_max_all_visibles['min_val']-(min_max_all_visibles['max_val']-min_max_all_visibles['min_val'])/10,
            min_max_all_visibles['max_val']+(min_max_all_visibles['max_val']-min_max_all_visibles['min_val'])/10
        ]); 
    }
}

function relayout_plotly_ts(data, id){
  if(data.hasOwnProperty('xaxis.range')){
    range_date[0] = data['xaxis.range'][0] ;
    range_date[1] = data['xaxis.range'][1] ;
    if(id == 'divSeriesPlotlyIndice'){
      generer_tableau_revision(range_date) ;
    }
    
    var new_data = {
      'xaxis.range': [data['xaxis.range'][0].substring(0,10), data['xaxis.range'][1].substring(0,10),],
    } ;
    var ids = ['divSeriesPlotlyIndice', 'divSeriesPlotlyEvol', 'divSeriesPlotlyGlissement'] ;
    ids.map(
      (x) => {
        var graphDiv = document.getElementById(x) ;
        
        var layoutRetrievedLater = graphDiv.layout ;
        if(
          (new_data['xaxis.range'][0] !== layoutRetrievedLater['xaxis']['range'][0]) ||
          (new_data['xaxis.range'][1] !== layoutRetrievedLater['xaxis']['range'][1])){
            new_data['yaxis.range'] = get_yaxis_range(graphDiv.data, range_date) ;
            Plotly.relayout(x, new_data) ;
        }
      }
    ) ;
  }
...
function legend_click_plotly_ts(data, id){
  ...
  if(id === 'divSeriesPlotlyIndice'){
    Plotly.restyle('divSeriesPlotlyEvol', update, [data.curveNumber]);
    Plotly.restyle('divSeriesPlotlyGlissement', update, [data.curveNumber]);
  }
  if(id === 'divSeriesPlotlyEvol'){
    Plotly.restyle('divSeriesPlotlyIndice', update, [data.curveNumber]);
    Plotly.restyle('divSeriesPlotlyGlissement', update, [data.curveNumber]);
  }
  if(id === 'divSeriesPlotlyGlissement'){
    Plotly.restyle('divSeriesPlotlyIndice', update, [data.curveNumber]);
    Plotly.restyle('divSeriesPlotlyGlissement', update, [data.curveNumber]);
  }
...

{
  ...
  var layout = {
    ...
    xaxis: {
      rangeslider: {yaxis: {rangemode: "auto", range: get_yaxis_range(traces, [dates[0].toISOString(), dates[dates.length-1].toISOString()])}},
      type: 'date',
      range: range_date
    }
  } ;
  ...
  div.setAttribute("id", "divSeriesPlotlyIndice");
  Plotly.newPlot(div, traces, layout, config);
  Plotly.relayout(div, {'yaxis.range': get_yaxis_range(traces, range_date)}) ;
  
  div.on('plotly_legendclick', function(data){
    legend_click_plotly_ts(data, 'divSeriesPlotlyIndice') ;
  });
  
  div.on('plotly_relayout', throttle(function(data){
    relayout_plotly_ts(data, 'divSeriesPlotlyIndice') ;
  }), 350);
...

{
  ...
  const div = DOM.element('div');
  div.setAttribute("id", "divSeriesPlotlyEvol");
  Plotly.newPlot(div, traces, layout, config);
  Plotly.relayout(div, {'yaxis.range': get_yaxis_range(traces, range_date)}) ;
  
  div.on('plotly_legendclick', function(data){
    legend_click_plotly_ts(data, 'divSeriesPlotlyEvol') ;
  });
  
  div.on('plotly_relayout', throttle(function(data){
    relayout_plotly_ts(data, 'divSeriesPlotlyEvol') ;
  }), 350);
...
//| echo: false
{
  ...
  const div = DOM.element('div');
  div.setAttribute("id", "divSeriesPlotlyGlissement");
  Plotly.newPlot(div, traces, layout, config);
  Plotly.relayout(div, {'yaxis.range': get_yaxis_range(traces, range_date)}) ;
  
  div.on('plotly_legendclick', function(data){
    legend_click_plotly_ts(data, 'divSeriesPlotlyGlissement') ;
  });
  
  div.on('plotly_relayout', throttle(function(data){
    relayout_plotly_ts(data, 'divSeriesPlotlyGlissement') ;
  }), 350);
...
//| echo: false
{
  ...
  var data_qualite = [
    {
      label: 'Modèle SARIMA',
      old_res: ((infos_old_sel === null) ? ' - ' : infos_old_sel['arima_model']),
      new_res: ((infos_new_sel === null) ? ' - ' : infos_new_sel['arima_model'])
    },
    {
      label: 'Transformation',
      old_res: ((infos_old_sel === null) ? ' - ' : (infos_old_sel['log'] === 1) ? 'Modèle multiplicatif' : 'Modèle additif'),
      new_res: ((infos_new_sel === null) ? ' - ' : (infos_new_sel['log'] === 1) ? 'Modèle multiplicatif' : 'Modèle additif')
    },
    {
      label:'Effet Pâques',
      old_res: ((infos_old_sel === null) ? ' - ' : infos_old_sel['easter'] == 1 ? String.fromCodePoint(0x2705) : String.fromCodePoint(0x274C)),
      new_res: ((infos_new_sel === null) ? ' - ' : infos_new_sel['easter'] == 1 ? String.fromCodePoint(0x2705) : String.fromCodePoint(0x274C))
    },
    {
      label:'Estimation span',
      old_res: ((infos_old_sel === null) ? ' - ' : format_span(infos_old_sel)['estimation_span']),
      new_res: ((infos_new_sel === null) ? ' - ' : format_span(infos_new_sel)['estimation_span'])
    },
  ] ;
  ...
  var data_qualite = [
    {
      ...
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['residuals_doornikhansen_pvalue'], infos_old_sel['residuals_doornikhansen_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['residuals_doornikhansen_pvalue'], infos_new_sel['residuals_doornikhansen_modality']))
    },
    {
      label: 'Kurtosis/Aplatissement',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['residuals_kurtosis_pvalue'], infos_old_sel['residuals_kurtosis_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['residuals_kurtosis_pvalue'], infos_new_sel['residuals_kurtosis_modality']))
    },
    {
      label: 'Skewness/Asymétrie',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['residuals_skewness_pvalue'], infos_old_sel['residuals_skewness_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['residuals_skewness_pvalue'], infos_new_sel['residuals_skewness_modality']))
    },
    {
      label: 'Indépendance des résidus<br>(Ljung-Box test)',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['residuals_lb_pvalue'], infos_old_sel['residuals_lb_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['residuals_lb_pvalue'], infos_new_sel['residuals_lb_modality']))
    },
    {
      label: 'Linéarité des résidus<br>(Ljung-Box test)',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['residuals_lb2_pvalue'], infos_old_sel['residuals_lb2_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['residuals_lb2_pvalue'], infos_new_sel['residuals_lb2_modality']))
    }
  ] ;
  ...
  var data_qualite = [
    {
      ...
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['qs_residuals_sa_on_sa_pvalue'], infos_old_sel['qs_residuals_sa_on_sa_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['qs_residuals_sa_on_sa_pvalue'], infos_new_sel['qs_residuals_sa_on_sa_modality']))
    },
    {
      label: 'Présence de saisonnalité résiduelle<br>dans la série CVS-CJO (F Test)',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['f_residuals_sa_on_sa_pvalue'], infos_old_sel['f_residuals_sa_on_sa_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['f_residuals_sa_on_sa_pvalue'], infos_new_sel['f_residuals_sa_on_sa_modality']))
    },
    {
      label: 'Présence de saisonnalité résiduelle<br>dans la composante irrégulière (QS Test)',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['qs_residuals_sa_on_i_pvalue'], infos_old_sel['qs_residuals_sa_on_i_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['qs_residuals_sa_on_i_pvalue'], infos_new_sel['qs_residuals_sa_on_i_modality']))
    },
    {
      label: 'Présence de saisonnalité résiduelle<br>dans la composante irrégulière (F Test)',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['f_residuals_sa_on_i_pvalue'], infos_old_sel['f_residuals_sa_on_i_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['f_residuals_sa_on_i_pvalue'], infos_new_sel['f_residuals_sa_on_i_modality']))
    },
    {
      label: 'Présence d\'effets de calendriers résiduels<br>dans la série CVS-CJO (F Test)',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['f_residuals_td_on_sa_pvalue'], infos_old_sel['f_residuals_td_on_sa_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['f_residuals_td_on_sa_pvalue'], infos_new_sel['f_residuals_td_on_sa_modality']))
    },
    {
      label: 'Présence d\'effets de calendriers résiduels<br>dans la composante irrégulière (F Test)',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['f_residuals_td_on_i_pvalue'], infos_old_sel['f_residuals_td_on_i_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['f_residuals_td_on_i_pvalue'], infos_new_sel['f_residuals_td_on_i_modality']))
    },
    {
      label: 'M7<br>Saisonnalité identifiable',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['m7'], infos_old_sel['m7_quality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['m7'], infos_new_sel['m7_quality']))
    },
    {
      label: 'Indicateur de qualité composite Q',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['q_stat'], infos_old_sel['q_stat_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['q_stat'], infos_new_sel['q_stat_modality']))
    },
    {
      label: 'Indicateur de qualité composite Q-M2',
      old_res: ((infos_old_sel === null) ? ' - ' : format_stat_jd(infos_old_sel['q_m2_stat'], infos_old_sel['q_m2_stat_modality'])),
      new_res: ((infos_new_sel === null) ? ' - ' : format_stat_jd(infos_new_sel['q_m2_stat'], infos_new_sel['q_m2_stat_modality']))
    }
  ] ;
...

{
  var data_si = (await db.query(`
      ...
      FROM (
        ...
        FROM (
          SELECT
            ...
            sum(CASE WHEN component ='s' THEN value ELSE null END) as s_new,
            sum(CASE WHEN component ='i' THEN value ELSE null END) as i_new,
          FROM jd_series
        ...
        FULL JOIN (
          SELECT
            ...
            sum(CASE WHEN component ='s' THEN value ELSE null END) as s_old,
            sum(CASE WHEN component ='i' THEN value ELSE null END) as i_old,
          FROM jd_series
      ...
      ORDER BY mois`,
      [
        ((infos_new_sel === null) ? 0 : infos_new_sel['log']),
        ((infos_new_sel === null) ? 0 : infos_new_sel['log']) ,
        ((infos_new_sel === null) ? '' : ws_new_sel),
        ((infos_new_sel === null) ? '' : sa_item_new_sel),
        ((infos_old_sel === null) ? 0 : infos_old_sel['log']) ,
        ((infos_old_sel === null) ? 0 : infos_old_sel['log']) ,
        ((infos_old_sel === null) ? '' : ws_old_sel),
        ((infos_old_sel === null) ? '' : sa_item_old_sel)
      ])) ;
